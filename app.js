...
// BNAPP Calendar – LUX Version B2
// לוח שנה עברי-לועזי עם שבת, חגים, מזג אוויר וסנכרון Firebase
...
const BNAPP = {
  ...
  shabbat: {},
  weatherCache: {},
  hebrewFormatter: new Intl.DateTimeFormat("he-IL-u-ca-hebrew", { day: "numeric", month: "long", year: "numeric" }),
  hebrewDayFormatter: new Intl.DateTimeFormat("he-IL-u-ca-hebrew", { day: "numeric" }),
  gregWeekday: new Intl.DateTimeFormat("he-IL", { weekday: "long" }),
  gregDate: new Intl.DateTimeFormat("he-IL", { day: "numeric", month: "long", year: "numeric" })
};
...
if (window.firebase && firebase.apps && firebase.apps.length) {
  ...
  try {
    ...
    console.warn("Firebase not initialised – check firebase-config.js");
  }
}
...
function toHebrewNumber(num) {
  ...
  if (tens === 1 && (ones === 5 || ones === 6)) {
    return ones === 5 ? "ט״ו" : "ט״ז";
  }
  if (tens > 0) {
    const tensMap = ["", "י", "כ", "ל", "מ", "נ", "ס", "ע", "פ", "צ"];
    out += tensMap[tens];
  }
  if (ones > 0) out += hebrewDigits[ones];
  if (out.length > 1) {
...
async function fetchHolidaysForMonth(year, month) {
  const m = month + 1;
  const url = `https://www.hebcal.com/hebcal?cfg=json&v=1&year=${year}&month=${m}&maj=on&min=on&mod=on&nx=on&mf=on&ss=on&c=on&geo=pos&latitude=${BNAPP.city.lat}&longitude=${BNAPP.city.lon}&tzid=${encodeURIComponent(
    BNAPP.city.tzid
  )}&lg=he`;
  try {
...
async function fetchShabbatForRange(startDate, endDate) {
  ...
  const url = `https://www.hebcal.com/shabbat?cfg=json&start=${startStr}&end=${endStr}&geo=pos&latitude=${BNAPP.city.lat}&longitude=${BNAPP.city.lon}&tzid=${encodeURIComponent(
    BNAPP.city.tzid
  )}&m=50&lg=he`;
  try {
    const res = await fetch(url);
...
async function fetchWeatherForDay(dateKey) {
  ...
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${BNAPP.city.lat}&longitude=${BNAPP.city.lon}&hourly=temperature_2m,precipitation_probability&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max&current_weather=true&timezone=auto&start_date=${start}&end_date=${start}`;
  try {
    const res = await fetch(url);
    const data = await res.json();
    const current = data.current_weather || {};
    const daily =
      data.daily && data.daily.time && data.daily.time.length
        ? {
...
function buildMonthMatrix(year, month) {
  const first = new Date(year, month, 1);
  const firstDayOfWeek = first.getDay(); // 0=ראשון, 6=שבת – תואם שורת הכותרת
...
async function renderCalendar() {
  ...
  document.getElementById("gregorianHeader").textContent = new Intl.DateTimeFormat("he-IL", {
    month: "long",
    year: "numeric"
  }).format(d0);
  ...
  document.getElementById("todayLabel").textContent = "היום: " + BNAPP.gregDate.format(BNAPP.today);
  ...
  cells.forEach(cell => {
    ...
    if (BNAPP.holidays[key] && BNAPP.holidays[key].length) {
      const candidates = BNAPP.holidays[key];
      let chosen = candidates.find(c =>
        /חג|ערב|חול המועד|ראש חודש|יום העצמאות|יום הכיפורים|פורים|פסח|סוכות|שבועות|חנוכה|תשעה באב|ט״ו בשבט|ל\"ג בעומר/.test(
          c.title
        )
      );
      if (!chosen) chosen = candidates[0];
      const t = document.createElement("div");
      t.className = "day-holiday";
    ...
    if (weekday === 5 && shab.candle && shab.candle.time) {
      ...
      sp.className = "shabbat-pill";
      sp.textContent = "🕯 " + shab.candle.time;
      tags.appendChild(sp);
    }
    if (weekday === 6 && shab.havdala && shab.havdala.time) {
      const sp = document.createElement("div");
      sp.className = "shabbat-pill";
      sp.textContent = "✨ " + shab.havdala.time;
      tags.appendChild(sp);
    }
    ...
    // חג ושבת גם כ"שורות" כדי שיראו יפה כמו משימות
    if (BNAPP.holidays[key] && BNAPP.holidays[key].length) {
      const holidayRow = document.createElement("div");
      holidayRow.className = "day-event-row";
      const dot = document.createElement("span");
      dot.className = "dot dot-both";
      const txt = document.createElement("span");
      const mainHoliday = BNAPP.holidays[key][0].title;
      txt.textContent = mainHoliday;
      holidayRow.appendChild(dot);
      holidayRow.appendChild(txt);
    ...
    if (weekday === 5 && shab.candle && shab.candle.time) {
      ...
      const dot = document.createElement("span");
      dot.className = "dot dot-benjamin";
      const txt = document.createElement("span");
      txt.textContent = "כניסת שבת " + shab.candle.time;
      row.appendChild(dot);
      row.appendChild(txt);
    ...
    if (weekday === 6 && shab.havdala && shab.havdala.time) {
      ...
      row.className = "day-event-row";
      const dot = document.createElement("span");
      dot.className = "dot dot-nana";
      const txt = document.createElement("span");
      txt.textContent = "יציאת שבת " + shab.havdala.time;
      row.appendChild(dot);
      row.appendChild(txt);
      content.appendChild(row);
    }
    ...
    const primaryEvents = userEvents.slice(0, 2);
    ...
    if (userEvents.length > 2) {
      ...
      more.textContent = `+${userEvents.length - 2} נוספים`;
...
async function openDayModal(dateKey) {
  ...
  if (BNAPP.holidays[dateKey] && BNAPP.holidays[dateKey].length) {
    holidayBox.textContent = BNAPP.holidays[dateKey].map(h => h.title).join(" • ");
  } else {
  ...
  function addEventRow(ev, isAuto = false) {
    ...
    const who =
      ev.owner === "benjamin" ? "בנימין" : ev.owner === "nana" ? "ננה" : "משותף";
    const kind = ev.kind === "task" ? "משימה" : "אירוע";
    meta.textContent = `${ev.start || ""}${ev.end ? "–" + ev.end : ""} • ${kind} • ${who}`;
  ...
  fetchWeatherForDay(dateKey).then(w => {
    ...
    weatherInline.textContent = `${emoji} ${Math.round(
      w.current.temperature || w.daily.max || 0
    )}°C  •  מקס' ${Math.round(
      w.daily.max || w.current.temperature || 0
    )}° / מינ' ${Math.round(w.daily.min || w.current.temperature || 0)}°`;
  });
...
function openEventModal(dateKey) {
  ...
  form.onsubmit = e => {
    ...
    const ev = {
      ...
      kind: document.getElementById("eventKind").value === "task" ? "task" : "event",
      type: "event",
      owner: document.getElementById("eventOwner").value,
      notify: document.getElementById("eventNotify").checked,
      notifyMinutes:
        parseInt(document.getElementById("eventNotifyMinutes").value, 10) || 60
    };
...
function openFreeTimeModal() {
  ...
  if (!free.length) content.textContent = "אין זמן חופשי היום 😅";
  else {
    content.innerHTML = free.map(r => `• ${r.start}–${r.end}`).join("<br>");
  }
  modal.classList.remove("hidden");
}
...
async function openWeatherModal() {
  ...
  sub.textContent = `מקסימום ${Math.round(
    daily.max || cur.temperature
  )}° / מינימום ${Math.round(daily.min || cur.temperature)}° • סיכוי גשם ${
    daily.pop || 0
  }%`;
